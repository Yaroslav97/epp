...
<div class="container">
	<div class="check-out">
        ...
        <c:forEach  items="${cartItems}" var="items">
		  ...
		  <tr>
		  <td class="ring-in"><a href="${path}${items.key.image}" class="at-in"><img src="${path}${items.key.image}" class="img-responsive" alt=""></a>
			<div class="sed">
				<h5>${items.key.name}</h5>
				<p>Type: ${items.key.subject}</p>
				<p>Origin: ${items.key.origin}</p>
			</div>
			<div class="clearfix"> </div></td>
			<td class="check"><input type="text" value="${items.value}" onfocus="this.value='';" onblur="if (this.value == '') {this.value ='';}"></td>
			<td>$${items.key.price}</td>
			<td>$${items.key.price * items.value}</td>
			<td><a href="/OnlineShop/removeItem?productName=${items.key.name}">remove item</a></td>
		  </tr>
...
<c:if test="${sessionScope.login != null}">
    <c:if test="${sessionScope.cartItems != null}">
	   ...
	   <a href="order.jsp" class="to-buy">Make order</a>
